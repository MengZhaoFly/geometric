...
tape("lineIntersectsLine(lineA, lineB) determines whether lineA intersects lineB", test => {
  ...
  test.equal(geometric.isLineCross(lineA, lineB), true);
  test.equal(geometric.isLineCross(lineA, lineC), false);
  test.equal(geometric.isLineCross(lineB, lineC), false);
  test.end();
});
...
tape("lineIntersectsLine(lineA, lineB) returns true if lineA and lineB share an endpoint", test => {
  ...
  test.equal(geometric.isLineCross(line1, line2), true);
  test.equal(geometric.isLineCross(line2, line1), true);
  test.end();
});